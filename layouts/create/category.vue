        ...
        >
          ...
          <div class="sm:col-span-3">
            <label
              ...
              class="inline-block text-sm font-medium text-white font-montserrat mt-2.5"
            >
              Выбрать родительскую категорию ( необязательно )
            </label>
          </div>
          <div class="sm:col-span-9">
    ...
    async createCategory() {
      try {
        ...
        if (this.selectedCategory) {
          payload.parent_id = this.selectedCategory;
        }

        console.log(payload)
        console.log(`${this.url}api/cp/categories/`)

        await axios.post(`${this.url}api/cp/categories/`, payload, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${this.token}`,
          },
